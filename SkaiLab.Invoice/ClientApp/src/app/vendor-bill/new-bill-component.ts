...
@Component({
    selector: 'new-bill-component',
    templateUrl: './new-bill-component.html',
    encapsulation: ViewEncapsulation.None
})
export class NewBillComponent extends OrganisationParentComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.bill.deliveryDate = new Date();
        this.showProgressBar();
        this.billService.getBillLookupForCreateOrUpdate().subscribe(result => {
            this.bill.orderNumber = result.orderNumber;
            this.bill.currencyId = result.baseCurrencyId;
            ...
            this.billLookup.locations = result.locations;
            ...
            this.billLookup.locations.splice(0, 0, location);
    ...
    constructor(private billService: BillService,
        ...
        super("New Bill",organisationService);
        this.setPageTitleFromLocalise(this.translate,"newBill");
        ...
        this.productDropdownConfig.searchOnKey = "name";
        this.ShowBackButton();
    }
    ...
    onRemoveAttachment(url: Attachment) {
        this.bill.attachments = this.bill.attachments.filter(obj => obj.fileUrl !== url.fileUrl);
    }
    ...
    onSaveApproval(isClose:boolean){
        ...
        modalRef.componentInstance.init(modalRef,"Bill: "+this.bill.orderNumber,hasCloseDoc);
    ...
    onSave(isClose:boolean,isPrint:boolean){
        ...
        this.showProgressBar();
        if(this.bill.id==0){
            this.billService.add(this.bill).subscribe(result=>{
                this.hideProgressBar();
                if(isPrint){
                    this.bill.id=result.id;
                    this.onPrint();
                }
                else{
                    if(isClose){
                        ...
                        this.bill=new Expense()
                        this.bill.orderNumber=result.orderNumber;
                        this.bill.currencyId=this.billLookup.baseCurrencyId;
                        this.selectedVendor=null;
                        this.calculateTotal();
                        this.billLookup.currencies.forEach(it=>{
                            if(this.bill.currencyId==it.id){
                                this.bill.currency=it;
                            }
                        });
                    }
